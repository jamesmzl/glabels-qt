...
/// Print
///
void PageRenderer::print( QPrinter* printer, QString outputFilename) const
{
    //QSizeF pageSize( mModel->tmplate()->pageWidth().mm() * 11.811, mModel->tmplate()->pageHeight().mm() * 11.811 );
    QSizeF pageSize( mModel->tmplate()->pageWidth().pt(), mModel->tmplate()->pageHeight().pt() );

    QSize size((int)pageSize.width() + 1, (int)pageSize.height() + 1);
    ...
    //QSizeF postekSize( mModel->tmplate()->pageWidth().mm() * 11.811, mModel->tmplate()->pageHeight().mm() * 11.811 );
    //img = img.scaled((int)postekSize.width() + 1, (int)postekSize.height() + 1);
...
void PageRenderer::printBmp(QString filename) const
{
    try
    {
        ...
        if ( mModel != nullptr )
        {
            ...
            model::Distance x = tmplate->pageWidth();
            model::Distance y = drawHeight - drawOffset;
            model::Distance w = tmplate->pageWidth();
            model::Distance h = drawHeight;
...
//            {
//                model::Distance x = origin.x();
//                model::Distance y = origin.y();
//                QPainterPath path = frame->path();
...
//            }
            ...
            //auto* overlayItem = new PreviewOverlayItem( this );
            //mScene->addItem( overlayItem );

            //get thumbnail
            QImage image((int)w.pt() + 1, (int)h.pt() + 1 ,QImage::Format_ARGB32);
            image.fill(Qt::white);
